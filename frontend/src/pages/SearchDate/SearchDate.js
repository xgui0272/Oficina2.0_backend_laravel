...
*/
  ...
  const numeroDePaginas = [];
     ...
     * filtrados pela data no banco de dados, e setar no array
     * de objetos budgets
     */
    useEffect(() => {
        const handleSearchDate = async () => {
          try {
            const response = await axios.get('http://127.0.0.1:8000/searchdate', {
              params: {
                startDate: dateStart,
                endDate: dateEnd
              },
    ...
    */
    return (
        <div className={styles.container}>
            <h1>Buscando por datas entre {data(dateStart)} até {data(dateEnd)}</h1>
            {loading ? (
                <h3>Carregando</h3>
            ) : (
